...
def dasilva_class(
    ...
    try:
        itertest = [
            iter(ni),
            iter(t),
        ]  # tests if inputs are iterable, if not "except" lines fire
        ...
        bias[:, 2] *= bvec[2]
        score = (
            np.dot(mat, np.log10(vec)).transpose() + bias
        )  # dot with matrix, add bias
...
def olshevsky_class(
    dist, model_file="../libraries/olshevsky_lib/cnn_dis_201711_verify.h5"
):
    """
...
def normalize_data(X, verbose=True):
    ...
    try:
        min_value = np.ma.masked_equal(
            X, 0.0, copy=False
        ).min()  # NOTE: this is modified to improve execution time/memory use. Original operation is np.min(X[np.nonzero(X)])
    except ValueError:
...
def class_query(
    t1, t2, class_df=None, datastore="ol_class.h5", key="ol_class", datapath="../data/"
):
    """
    ...
    ind1 = round(
        np.interp(t1, class_df["time"].to_numpy(), np.arange(len(class_df)))
    )  # Closest index for t1
    ind2 = round(
        np.interp(t2, class_df["time"].to_numpy(), np.arange(len(class_df)))
    )  # Closest index for t2
...
def translate_labels(label, classifier="dasilva"):
    ...
    if classifier == "dasilva":
        label_new = np.where(
            (label == 0) | (label == 1), (label + 1) % 2, label
        )  # This changes 0s to 1s and vice versa
    if classifier == "olshevsky":
        label_new = (label + 2) % 4  # This changes 0->2, 1->3, 2->0, 3-1
        label_new = np.where(
            (label_new == 0) | (label_new == 1), (label_new + 1) % 2, label_new
        )  # This changes 0s to 1s and vice versa
    return label_new
...
def translate_probabilities(probability):
    ...
    """
    new_probability = np.zeros(
        probability.shape
    )  # Make an empty array in the shape of probability array
...
def get_cdas_keys(instrument, dtype, sc, rate):
    ...
    if instrument == "fpi":
        ...
        keys = [
            sc + "_" + dtype[0:3] + "_bulkv_gse_" + rate,
            sc + "_" + dtype[0:3] + "_numberdensity_" + rate,
            sc + "_" + dtype[0:3] + "_errorflags_" + rate,
        ]
    if instrument == "fgm":
        if rate == "srvy":
...
def load_util(start_date, end_date, instrument, dtype, sc, rate, freq="1D"):
    ...
    dates = (
        pd.date_range(start_date, end_date + pd.Timedelta(freq), freq=freq)
        .strftime("%Y-%m-%d")
        .tolist()
    ...
    for idx, date in enumerate(dates):
        if idx == len(dates) - 1:  # If we're at the last date, break
            break
        dataset, keys = get_cdas_keys(
            instrument, dtype, sc, rate
        )  # Get the name of the dataset in CDAS and the CDF keys to load
        ...
        if instrument == "fpi":
            ...
            for key in keys:
                if (
                    key != sc + "_" + dtype[0:3] + "_bulkv_gse_" + rate
                ):  # Bulk velocity is three components, so we need to split it up in the else statement
        ...
        if instrument == "fgm":
            ...
            for key in keys:
                if (
                    key != sc + "_" + instrument + "_b_gsm_" + rate + "_l2"
                ):  # Magnetic field is three components, so we need to split it up in the else statement
                    df_stage[key] = data[1][key]
                else:
                    df_stage[key + "_x"] = data[1][key][:, 0]
                    df_stage[key + "_y"] = data[1][key][:, 1]
            ...
            keys = [
                "Epoch",
                sc + "_" + instrument + "_r_gsm",
                sc + "_" + instrument + "_r_gse",
            ]
            df_stage["Epoch"] = data[1]["Epoch"]
            for key in keys:
                if (key != "Epoch", sc + "_" + instrument + "_r_gsm") & (
                    key != sc + "_" + instrument + "_r_gse"
                ):  # Position is three components, so we need to split it up in the else statement
                    df_stage[key] = data[1][key]
                else:
                    df_stage[key + "_x"] = data[1][key][:, 0]
                    df_stage[key + "_y"] = data[1][key][:, 1]
...
def clean_columns(raw_data, instrument, dtype, rate, sort=False):
    ...
    if instrument == "fpi":
        ...
        clean_df.rename(
            columns={
                "mms1_" + dtype[0:3] + "_numberdensity_" + rate: "n_" + dtype[1],
                "mms1_"
                + dtype[0:3]
                + "_bulkv_gse_"
                + rate
                ...
                + dtype[1]
                + "z_gse",
                "mms1_" + dtype[0:3] + "_temppara_" + rate: "T" + dtype[1] + "_para",
                "mms1_" + dtype[0:3] + "_tempperp_" + rate: "T" + dtype[1] + "_perp",
                "mms1_" + dtype[0:3] + "_errorflags_" + rate: "eflags",
            },
            inplace=True,
        )
        clean_df["Epoch"] = pd.to_datetime(
            raw_data["Epoch"], utc=True
        )  # Convert to UTC aware datetime
    elif instrument == "fgm":
        clean_df = raw_data.copy()
        clean_df.rename(
            columns={
                "mms1_" + instrument + "_b_gsm_" + rate + "_l2_x": "Bx_gsm",
                "mms1_" + instrument + "_b_gsm_" + rate + "_l2_y": "By_gsm",
                "mms1_" + instrument + "_b_gsm_" + rate + "_l2_z": "Bz_gsm",
                "mms1_" + instrument + "_b_gsm_" + rate + "_l2_mag": "B_gsm",
                "mms1_" + instrument + "_flag_" + rate + "_l2": "eflags",
            },
            inplace=True,
        )
        clean_df["Epoch"] = pd.to_datetime(
        ...
        clean_df.rename(
            columns={
                "mms1_" + instrument + "_r_gsm_x": "Px_gsm",
                "mms1_" + instrument + "_r_gsm_y": "Py_gsm",
                "mms1_" + instrument + "_r_gsm_z": "Pz_gsm",
                "mms1_" + instrument + "_r_gse_x": "Px_gse",
                "mms1_" + instrument + "_r_gse_y": "Py_gse",
                "mms1_" + instrument + "_r_gse_z": "Pz_gse",
            },
            inplace=True,
        )
        clean_df["Epoch"] = pd.to_datetime(
...
def mms_load_util(
    start_date, end_date, keys, clear_data=False, datapath="../data/pydata"
):
    """
    ...
    fpi_df = pd.DataFrame(
        dtype=object
    )  # Initialize empty dataframes to put each type of data
    ...
    for key in keys:
        if (
            (key == "mms1_dis_bulkv_gse_fast")
            | (key == "mms1_dis_numberdensity_fast")
            | (key == "mms1_dis_tempperp_fast")
            | (key == "mms1_dis_energyspectr_omni_fast")
        ):
            if not fpi_i_load:
                fpi_i_data = mms.fpi(
                    trange=load_trange,
                ...
                fpi_t_arr = (
                    pytplot.data_quants[key].coords["time"].values
                )  # FPI Timestamp
            ...
            if (
                key == "mms1_dis_bulkv_gse_fast"
            ):  # The velocity data is actually in three component vectors
        ...
        if key == "mms1_mec_r_gse":
            if not mec_load:
                ...
                mec_t_arr = (
                    pytplot.data_quants["mms1_mec_r_gse"].coords["time"].values
                )  # MEC Timestamp
                mec_df["time"] = mec_t_arr  # Store the time in the dataframe
                mec_load = True  # We've loaded the data! Don't do it again!
            pos_arr = (
                pytplot.data_quants["mms1_mec_r_gse"].values / EARTH_RADIUS
            )  # GSE Position (RE)
        ...
        if key == "ol_class":
            cla_df = class_query(
                start_date.timestamp(), end_date.timestamp(), datapath=datapath
            )
        try:
...
def omni_shifter(omni_data, mms_data):
    ...
    for i in np.arange(len(mms_data)):
        ...
        delta_t = (nx * (Rdx - Rox) + ny * (Rdy - Roy) + nz * (Rdz - Roz)) / (
            (nx * Vx) + (ny * Vy) + (nz * Vz)
        )

        omni_ind[i] = np.argmin(
            np.abs(omni_time + delta_t - mms_time)
...
def Em_err(vx, vx_sig, vy, vy_sig, vz, vz_sig, by, by_sig, bz, bz_sig):
    ...
    dEmvx = (
        ...
        * (vx**2 + vy**2 + vz**2) ** (-1 / 2)
        * (by**2 + bz**2) ** (1 / 2)
        * (np.sin(np.arctan2(by, bz)) ** 2)
    )  # Partial derivative of Em wrt Vx
    dEmvy = (
        vy
        * (vx**2 + vy**2 + vz**2) ** (-1 / 2)
        * (by**2 + bz**2) ** (1 / 2)
        * (np.sin(np.arctan2(by, bz)) ** 2)
    )  # Partial derivative of Em wrt Vy
    dEmvz = (
        vz
        * (vx**2 + vy**2 + vz**2) ** (-1 / 2)
        * (by**2 + bz**2) ** (1 / 2)
        * (np.sin(np.arctan2(by, bz)) ** 2)
    )  # Partial derivative of Em wrt Vz
    dEmby = (
        by
        * (vx**2 + vy**2 + vz**2) ** (1 / 2)
        * (by**2 + bz**2) ** (-1 / 2)
        ...
        * (by / (bz**2))
        / ((by**2) / (bz**2) + 1) ** 2
    )  # Partial derivative of Em wrt By
    dEmbz = (
        bz
        * (vx**2 + vy**2 + vz**2) ** (1 / 2)
        * (by**2 + bz**2) ** (-1 / 2)
        ...
        * ((by**2) / (bz**3))
        / ((by**2) / (bz**2) + 1) ** 2
    )  # Partial derivative of Em wrt Bz
    Em_err = np.sqrt(
        (dEmvx * vx_sig) ** 2
        + (dEmvy * vy_sig) ** 2
        + (dEmvz * vz_sig) ** 2
...
def pdyn_err(n, vx, n_sig, vx_sig):
    ...
    """
    err = np.sqrt(
        (n_sig * vx**2 * 1.673e-6) ** 2 + (n * vx * vx_sig * 2 * 1.673e-6) ** 2
...
def mag_err(x, x_sig, y, y_sig, z, z_sig):
    ...
    """
    err = np.sqrt((x * x_sig) ** 2 + (y * y_sig) ** 2 + (z * z_sig) ** 2) / np.sqrt(
        x**2 + y**2 + z**2
    )  # Add errors in quadrature
    return err
...
def theta_err(y, y_sig, z, z_sig):
    err = np.sqrt((y * y_sig) ** 2 + (z * z_sig) ** 2) / (
        y**2 + z**2
    )  # Add errors in quadrature
    return err
...
def index_shifter(in_df, shift, keys):
    ...
    shift_index = shift_index[
        (shift_index >= 0) & (shift_index < len(in_df))
...
def fillutil(cdf_struct, varname, attdict, data):
    ...
    if np.min(data) >= np.max(
        data
    ):  # If the data is constant/length 1, we need to set VALIDMIN and VALIDMAX to almost the same value
        ...
        try:
            ...
            cdf_struct[varname].attrs["VALIDMAX"] = np.min(data) + pd.Timedelta(
                seconds=1
            )
    else:
        cdf_struct[varname].attrs["VALIDMIN"] = np.min(data)
...
def cdfw(data, filename):
    ...
    * write : bool
    ...
    fillutil(
        cdf,
        "B_GSM",
        cdfd.bgsm_primebsn_att,
        data.loc[:, ["B_xgsm", "B_ygsm", "B_zgsm"]].to_numpy(),
    )
    fillutil(
        cdf,
        "B_GSM_sig",
        cdfd.bgsmsig_primebsn_att,
        data.loc[:, ["B_xgsm_sig", "B_ygsm_sig", "B_zgsm_sig"]].to_numpy(),
    )
    fillutil(
        cdf,
        "V_GSE",
        cdfd.vgse_primebsn_att,
        data.loc[:, ["Vi_xgse", "Vi_ygse", "Vi_zgse"]].to_numpy(),
    )
    fillutil(
        cdf,
        "V_GSE_sig",
        cdfd.vgsesig_primebsn_att,
        data.loc[:, ["Vi_xgse_sig", "Vi_ygse_sig", "Vi_zgse_sig"]].to_numpy(),
    )
    ...
    labelutil(
        cdf, "B_GSM_label", cdfd.bgsm_primebsn_label, ["Bx GSM", "By GSM", "Bz GSM"]
    )
    labelutil(
        cdf,
        "B_GSM_sig_label",
        cdfd.bgsmsig_primebsn_label,
        ["Bx GSM Sigma", "By GSM Sigma", "Bz GSM Sigma"],
    )
    labelutil(
        cdf, "V_GSE_label", cdfd.vgse_primebsn_label, ["Vx GSE", "Vy GSE", "Vz GSE"]
    )
    labelutil(
        cdf,
        "V_GSE_sig_label",
        cdfd.vgsesig_primebsn_label,
        ["Vx GSE Sigma", "Vy GSE Sigma", "Vz GSE Sigma"],
    )
    ...
    labelutil(
        cdf, "V_GSE_sig_units", cdfd.vgsesig_primebsn_units, ["km/s", "km/s", "km/s"]
    )
