...
class prime(ks.Model):
    ...
    def predict(self, input):
        ...
        if isinstance(input, pd.DataFrame):  # If input is a dataframe
            input_arr = input[
                self.in_keys
            ].to_numpy()  # Convert input dataframe to array
        ...
        output = pd.DataFrame(
            output_arr, columns=self.out_keys
        )  # Convert output array to dataframe
        output_epoch = input["Epoch"].to_numpy()[
            (self.window - 1) :
        ]  # Stage an epoch column to be added to the output dataframe
        output_epoch += pd.Timedelta(
            seconds=100 * self.stride
        )  # Add lead time to the epoch column
    ...
    def predict_raw(self, input):
        ...
        input_arr = np.zeros(
            (len(input_scaled) - (self.window - 1), self.window, len(self.in_keys))
        )  # Reshape input data to be 3D
        for i in np.arange(len(input_scaled) - (self.window - 1)):
            input_arr[i, :, :] = input_scaled[
                i : (i + self.window)
            ]  # Move the 55 unit window through the input data
        output_unscaled = self.model.predict(input_arr)
        output = np.zeros(
            (len(output_unscaled), len(self.out_keys))
        )  # Stage output data to be 2x target dimensions
        output[:, ::2] = self.tar_scaler.inverse_transform(
            output_unscaled[:, ::2]
        )  # Mean values
        output[:, 1::2] = np.abs(
            self.tar_scaler.inverse_transform(
                output_unscaled[:, ::2] + output_unscaled[:, 1::2]
            )
            - self.tar_scaler.inverse_transform(output_unscaled[:, ::2])
        )  # Standard deviations
        return output
    ...
    def predict_grid(
        ...
        """
        x_arr = np.arange(
            x_extent[0], x_extent[1], gridsize
        )  # Create a grid to calculate the magnetosheath conditions on
        ...
        if y_extent is not None:
            y_arr = np.arange(
                y_extent[0], y_extent[1], gridsize
            )  # Create a grid to calculate the magnetosheath conditions on
        if z_extent is not None:
            y_arr = np.arange(
                z_extent[0], z_extent[1], gridsize
            )  # Create a grid to calculate the magnetosheath conditions on (called y but contains z because im lazy)
        x_grid, y_grid = np.meshgrid(
            x_arr, y_arr
        )  # Create a grid to calculate the magnetosheath conditions on
        input_seed = np.zeros(
            (len(x_grid.flatten()) * framenum, len(self.in_keys))
        )  # Initialize array to hold the input data before unfolding it
        for idx, element in enumerate(
            [bx, by, bz, vx, vy, vz, ni, vt, rx, ry, rz]
        ):  # Loop through the input data and repeat it
            try:
                iter(element)  # Check if the element is iterable
                input_seed[:, idx] = np.repeat(
                    element, len(x_grid.flatten())
                )  # If it is, repeat it for each grid point
            except (
                TypeError
            ):  # This error throws if iter(element) fails (i.e. element is not iterable)
                input_seed[:, idx] = np.repeat(
                    element, framenum * len(x_grid.flatten())
                )  # If it isn't, repeat it for each grid point *and frame*
        loc_arr = np.zeros(
            (len(x_grid.flatten()) * framenum, 3)
        )  # Initialize array to hold the location data
        ...
        input_seed_scaled[:, 11:14] = self.loc_scaler.transform(
            loc_arr
        )  # Scale the location data
        input_seed_scaled = np.repeat(
            input_seed_scaled, self.window, axis=0
        )  # Repeat the input data 55 times to make static timeseries
        input_arr = input_seed_scaled.reshape(
            len(x_grid.flatten()) * framenum, self.window, len(self.in_keys)
        )  # Reshape the input data into the correct shape
        output_arr = self.model.predict(input_arr)  # Predict the output data
        output = np.zeros(
            (len(output_arr), len(self.out_keys))
        )  # Stage output data to be 2x target dimensions
        output[:, ::2] = self.tar_scaler.inverse_transform(
            output_arr[:, ::2]
        )  # Mean values
        output[:, 1::2] = np.abs(
            self.tar_scaler.inverse_transform(output_arr[:, ::2] + output_arr[:, 1::2])
            - self.tar_scaler.inverse_transform(output_arr[:, ::2])
        )  # Standard deviations
        output_grid = output.reshape(
            framenum, len(y_arr), len(x_arr), len(self.out_keys)
        )  # Reshape the output data into the correct shape
        output_grid = np.swapaxes(
            output_grid, 1, 2
        )  # Move the y axis to the second axis
        if loc_mask:
            bs_array = np.zeros(
                (framenum, len(y_arr))
            )  # Initialize array to hold the bow shock locations
            mp_array = np.zeros(
                (framenum, len(y_arr))
            )  # Initialize array to hold the magnetopause locations
            # Make a mask for all points outside the bow shock or inside the magnetopause
            output_mask = np.zeros(
                output_grid.shape, dtype=bool
            )  # Initialize array to hold the frame mask
            for i in np.arange(framenum):
                if (
                    framenum == 1
                ):  # If there is only one frame turn pdyn and bz into iterables
        ...
        if (
            subtract_ecliptic
        ):  # If subtract_ecliptic is true, subtract the Earth's motion in the ecliptic from Vy
            output_grid[:, :, :, 8] -= 29.8
        return output_grid
    ...
    def build_model(
        self, units=[416, 352, 32, 64], activation="elu", dropout=0.35, lr=1e-4
    ):
        """
...
def streamline(
    ...
    if data is not None:
        im = axes.imshow(
            data,
            origin="lower",
            extent=[y_extent[0], y_extent[1], x_extent[0], x_extent[1]],
            aspect="equal",
            cmap=cmap,
            vmin=vmin,
            vmax=vmax,
        ...
        im = axes.imshow(
            frames[frame_index, :, :, param_index],
            origin="lower",
            extent=[y_extent[0], y_extent[1], x_extent[0], x_extent[1]],
            aspect="equal",
            cmap=cmap,
    ...
    if draw_streamline:
        ...
        stream = ax2.streamplot(
            y_grid,
            x_grid,
            frames[frame_index, :, :, v_index],
            frames[frame_index, :, :, u_index],
            color=linecolor,
            density=density,
            linewidth=1,
        )
...
def colorbar_maker(
    fig, im, param_index=None, label=None, box=[0.92, 0.55, 0.015, 0.33]
):
    """
    ...
    if param_index is not None:
        labels = [
            ...
            r"$B_{Z}$ GSE $\sigma$ (nT)",
            r"$V_{X}$ GSE (km/s)",
            r"$V_{X}$ GSE $\sigma$ (km/s)",
            r"$V_{Y}$ GSE (km/s)",
            r"$V_{Y}$ GSE $\sigma$ (km/s)",
            r"$V_{Z}$ GSE (km/s)",
            r"$V_{Z}$ GSE $\sigma$ (km/s)",
            r"$n_{i}$ ($cm^{-3}$)",
            r"$n_{i}$ $\sigma$ ($cm^{-3}$)",
            r"$T_{i\perp}$ (eV)",
            r"$T_{i\perp}$ $\sigma$ (eV)",
            r"$T_{i\parallel}$ (eV)",
            r"$T_{i\parallel}$ $\sigma$ (eV)",
        ]
...
def crps_f(ep, sg):
    ...
    """
    crps = sg * (
        (ep / sg) * tf.math.erf((ep / (np.sqrt(2) * sg)))
        + tf.math.sqrt(2 / np.pi) * tf.math.exp(-(ep**2) / (2 * sg**2))
        - 1 / tf.math.sqrt(np.pi)
    )
    return crps
...
def unstack_helper(y_true, y_pred):
    ...
    (
        ...
        mu8,
        sg8,
    ) = tf.unstack(y_pred, axis=-1)
    # Oh my god mu2, I can't believe it

    # Separate the ground truth into each parameter
    y_true0, y_true1, y_true2, y_true3, y_true4, y_true5, y_true6, y_true7, y_true8 = (
        tf.unstack(y_true, axis=-1)
    )
