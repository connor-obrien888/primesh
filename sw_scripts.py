...
def class_load(
    ...
    end_delt = pd.to_datetime(end) + pd.Timedelta(
        load_freq
    )  # Get the last date of the date range to ensure the end is included
    dates = (
        pd.date_range(start, end_delt, freq=load_freq)
        .strftime("%Y-%m-%d %H:%M:%S+0000")
        .tolist()
    )  # 1 day intervals from 2015 to 2023 (inclusive)
    ...
    for i in np.arange(len(dates) - 1):
        ...
        try:
            ...
            dist_arr = np.asarray(
                data[1]["mms1_dis_dist_fast"]
            )  # Pull the 3D distribution
            label, probability = sw.olshevsky_class(dist_arr)  # Classify the full array
            p_arr = sw.translate_probabilities(
                probability
            )  # Store the class probabilities
            f_arr = sw.translate_labels(
                label, classifier="olshevsky"
            )  # Region flags (see translate_labels)
            ...
            class_df = class_df.append(
                class_df_stage, ignore_index=True
            )  # Append this day to the full dataframe
            del (
                data,
                dist_arr,
                label,
...
def inter_saver(
    storename="mms_data",
    class_key="ol_class",
    inter_key="stable_intervals",
    prob_threshold=0.7,
    time_threshold=pd.Timedelta("4.6s"),
    datapath="../data/",
):
    """
    ...
    class_df["Epoch"] = pd.to_datetime(
        class_df["Epoch"], utc=True
    )  # Add timezone information to the time array
    intervals = pd.DataFrame(
        columns=["start", "end", "region"]
    )  # Create a dataframe to hold the start and end times of the intervals
    saving = True  # Set a flag to indicate that we are saving intervals
    start = class_df["Epoch"][
        0
    ]  # Set the start time to the first time in the dataframe
    for i in range(1, len(class_df)):
        print(
            "Analyzing " + str(i / len(class_df) * 100)[0:4] + "% Complete", end="\r"
        )  # Print the percentage complete
        if (
            (class_df["region"][i] == class_df["region"][i - 1])
            & (class_df["Epoch"][i] - class_df["Epoch"][i - 1] < time_threshold)
            & (
                np.max(class_df.loc[i, ["MSP_p", "MSH_p", "SW_p", "IF_p"]])
                > prob_threshold
            )
        ):  # Are we in the same region with no outages?
            if saving == False:  # Are we not saving intervals?
                start = class_df["Epoch"][
                    i - 1
                ]  # Set the start time to the previous time
            ...
            if saving == True:  # Are we saving intervals?
                ...
                intervals = intervals.append(
                    {"start": start, "end": end, "region": class_df["region"][i - 1]},
                    ignore_index=True,
                )  # Add the interval to the dataframe
                saving = (
                    False  # Set the flag to indicate that we are not saving intervals
                )
    intervals.to_hdf(
        datapath + storename + ".h5", key=inter_key, mode="a"
    )  # Save the intervals to the HDF5 file
...
def bin_maker(
    ...
    for i in range(len(class_intervals)):
        ...
        bins_stage = pd.date_range(
            start=class_intervals["start"][i],
            end=class_intervals["end"][i],
            freq="100s",
        )
        bins_df_stage = pd.DataFrame([])
        bins_df_stage["start"] = bins_stage[:-1]
        bins_df_stage["end"] = bins_stage[1:]
        bins_df_stage["region"] = (
            np.ones(len(bins_df_stage)) * class_intervals["region"][i]
        )
...
def dis_load(
    ...
    load_freq: Frequency of the data to load (e.g. '6M' for 6 months. DIS data is pretty sparse, so
    this can be a large value)
    ...
    for i in range(len(dates)):
        ...
        try:
            data = cdas.get_data(
                ...
                ["mms1_dis_bulkv_gse_fast", "mms1_dis_numberdensity_fast"],
                dates[i],
                dates[i + 1],
            )
    ...
    for i in np.arange(len(dates) - 1):
        ...
        try:
            data = cdas.get_data(
                ...
                ["mms1_mec_r_gsm_srvy_l2", "mms1_mec_r_gse_srvy_l2"],
                dates[i],
                dates[i + 1],
            )
    ...
    for i in range(len(dates) - 1):
        try:
            ...
            print(
                ...
                + " of "
                + str(len(dates))
                + " has corrupted FGM data"
            )
            continue
        times = pd.date_range(
            dates[i], dates[i + 1], freq=bin_freq
        )  # Five second intervals for the day
        bin_subset = pd.IntervalIndex.from_arrays(
            times[:-1], times[1:], closed="left"
        )  # Get the bins for the current day
        fgm_group = fgm_data.groupby(
            pd.cut(fgm_data["Epoch_fgm"], bin_subset)
        )  # Bin the FGM data
        fgm_binned_stage = fgm_group.mean()  # Get the mean of the binned data
        fgm_binned_stage["count"] = fgm_group.count()[
            "Bx_gsm"
        ]  # Get the number of data points in each bin
        fgm_binned_stage["Epoch_fgm"] = (
            bin_subset.left
        )  # Add the start of the bins as the Epoch
        fgm_df = fgm_df.append(
            fgm_binned_stage, ignore_index=True
...
def swe_load(
    ...
    for i in np.arange(len(dates) - 1):
        ...
        try:
            data = cdas.get_data(
                "WI_K0_SWE",
                ["Np", "V_GSE", "THERMAL_SPD", "QF_V", "QF_Np"],
                dates[i],
                dates[i + 1],
            )
    ...
    swe_df["Epoch"].where(
        swe_df["Epoch"] >= pd.to_datetime(dates[0], utc=True), np.nan, inplace=True
    )
    swe_df["Epoch"].where(
        swe_df["Epoch"] <= pd.to_datetime(dates[-1], utc=True), np.nan, inplace=True
    )
...
def mms_integrator(
    storename="mms_data", key="targets", datapath="../data/", verbose=False
):
    """
    ...
    if verbose:
        print("Finished adding region column")
    # Combine the three dataframes
    mms_data = pd.merge(
        mec_binned.drop(columns="region"),
        des_binned.drop(columns="region"),
        left_index=True,
        right_index=True,
    )
    mms_data = pd.merge(
        mms_data, dis_binned.drop(columns="region"), left_index=True, right_index=True
    )
    mms_data = pd.merge(
        mms_data, fgm_binned.drop(columns="region"), left_index=True, right_index=True
    )
    ...
    mms_data[["R_xgsm", "R_ygsm", "R_zgsm", "R_xgse", "R_ygse", "R_zgse"]] = (
        mms_data[["R_xgsm", "R_ygsm", "R_zgsm", "R_xgse", "R_ygse", "R_zgse"]] / 6378
    )  # Convert the R vectors to be in units of Earth radii
    # Check if MMS data was obtained with SW energy-azimuth table
    mms_data["Vi_ygse"][
        mms_data["SW_table"] == True
    ] += (
        -10.7
    )  # Subtract 10.7 km/s to Vy_gse if MMS data was obtained with SW energy-azimuth table
    mms_data["Vi_ygse"][
        mms_data["SW_table"] == False
    ] += (
        -29.0
    )  # Subtract 29.0 km/s to Vy_gse if MMS data was not obtained with SW energy-azimuth table
    mms_data.rename(
        columns={
            "Bx_gse": "B_xgse",
            "By_gse": "B_ygse",
...
def swmf_input(data, start_time, window, cadence, filename, datapath="../data/"):
    ...
    with open("test.dat", "r+") as f:
        ...
        f.write(
            header.rstrip("\r\n") + "\n" + content[71:]
        )  # 71 is the number of characters in the header from pandas (e.g. 'year', 'month', etc.)
...
def wind_ascii_prep():
    ...
    """
    wind_2015 = pd.read_csv(
        "../data/omni_data/wind_ascii/2015wind.txt",
        delim_whitespace=True,
        names=sw.WIND_ASCII_COLS,
    )
    wind_2016 = pd.read_csv(
        "../data/omni_data/wind_ascii/2016wind.txt",
        delim_whitespace=True,
        names=sw.WIND_ASCII_COLS,
    )
    wind_2017 = pd.read_csv(
        "../data/omni_data/wind_ascii/2017wind.txt",
        delim_whitespace=True,
        names=sw.WIND_ASCII_COLS,
    )
    wind_2018 = pd.read_csv(
        "../data/omni_data/wind_ascii/2018wind.txt",
        delim_whitespace=True,
        names=sw.WIND_ASCII_COLS,
    )
    wind_2019 = pd.read_csv(
        "../data/omni_data/wind_ascii/2019wind.txt",
        delim_whitespace=True,
        names=sw.WIND_ASCII_COLS,
    )
    wind_2020 = pd.read_csv(
        "../data/omni_data/wind_ascii/2020wind.txt",
        delim_whitespace=True,
        names=sw.WIND_ASCII_COLS,
    )
    wind_2021 = pd.read_csv(
        "../data/omni_data/wind_ascii/2021wind.txt",
        delim_whitespace=True,
        names=sw.WIND_ASCII_COLS,
    )
    wind_2022 = pd.read_csv(
        "../data/omni_data/wind_ascii/2022wind.txt",
        delim_whitespace=True,
        names=sw.WIND_ASCII_COLS,
    )
    wind_full = wind_2015.append(
        [wind_2016, wind_2017, wind_2018, wind_2019, wind_2020, wind_2021, wind_2022],
        ignore_index=True,
    )
    ...
    for i in np.arange(len(wind_full)):
        date.append(
            pd.to_datetime(
                str(int(wind_full["year"][i]))
                + "-"
                + str(int(wind_full["doy"][i]))
                + "-"
                + str(int(wind_full["hour"][i]))
                + "-"
                + str(int(wind_full["minute"][i])),
                format="%Y-%j-%H-%M",
                utc=True,
            )
        )
...
def omni_shift(
    wind_store="wind_data",
    wind_key="wind_omni_bs",
    mms_store="mms_data",
    mms_key="targets",
    key="wind_omni_shift",
    datapath="../data/",
    ...
    omni_time = (
        wind_data["Epoch"] - pd.Timestamp("1970-01-01 00:00:00+0000")
    ) // pd.Timedelta(
        "1s"
    ...
    denominator = (
        (nx * Vx) + (ny * Vy) + (nz * Vz)
    )  # Denominator of time delay function
    ...
    for i in np.arange(len(mms_data)):
        mms_time = (
            mms_data["Epoch"][i] - pd.Timestamp("1970-01-01 00:00:00+0000")
        ) // pd.Timedelta(
            "1s"
        )  # Convert to seconds since epoch
        cut_bool = wind_data["Epoch"].between(
            mms_data["Epoch"][i] - pd.Timedelta("1D"), mms_data["Epoch"][i]
        )  # Boolean array of omni data within 1 day on either side of MMS data
        min_ind = cut_bool[
            cut_bool
        ].index.min()  # Find the lowest index so we can store the right one later

        Rdx = (
            mms_data["R_xgse"][i] * sw.EARTH_RADIUS
        )  # MMS position (Convert from Re to km)
        Rdy = mms_data["R_ygse"][i] * sw.EARTH_RADIUS
        Rdz = mms_data["R_zgse"][i] * sw.EARTH_RADIUS

        delta_t = (
            nx[cut_bool] * (Rdx - Rox[cut_bool])
            + ny[cut_bool] * (Rdy - Roy[cut_bool])
            + nz[cut_bool] * (Rdz - Roz[cut_bool])
        ) / denominator[
            cut_bool
        ]  # OMNI algorithm time delay function
        try:
            omni_ind[i] = (
                np.argmin(np.abs(omni_time[cut_bool] + delta_t - mms_time)) + min_ind
            )  # which piece of omni_data corresponds to this adjusted time?
            shift[i] = np.min(np.abs(omni_time[cut_bool] + delta_t - mms_time))
        except (
            ValueError
        ):  # Throws when there's no data in omni_data within 1 day on either side of MMS data
            omni_ind[i] = (
                0  # Unfortunately nan values don't work with pandas indexing. Look for nan timeshifts to drop data later
            )
            shift[i] = (
                np.nan
            )  # Unfortunately nan values don't work with pandas indexing. Look for nan timeshifts to drop data later
        print(
            "OMNI Propagation " + str(100 * i / len(mms_data))[0:5] + "% Complete",
            end="\r",
        )
...
def omni_fillval(
    omni_data, B_fill=9999.99, V_fill=99999.9, n_fill=999.99, T_fill=9999999
):
    """
    ...
    omni_data["proton_density"].where(
        omni_data["proton_density"] != n_fill, np.nan, inplace=True
    )
...
def nn_omni_comp_plot(
    ...
    mms_data_cut = mms_data[
        (mms_data["Epoch"] >= trange[0]) & (mms_data["Epoch"] <= trange[1])
    ]
    predict_cut = predict[
        (predict["Epoch"] >= trange[0]) & (predict["Epoch"] <= trange[1])
    ]
    ...
    for i in np.arange(1, len(predict_cut)):
        if (
            predict_cut["Epoch"].iloc[i] - predict_cut["Epoch"].iloc[i - 1]
        ) > pd.Timedelta("600s"):
            bonus_epochs = bonus_epochs.append(
                {"Epoch": predict_cut["Epoch"].iloc[i - 1] + pd.Timedelta("100s")},
                ignore_index=True,
            )
    ...
    if omni_data is not None:
        omni_cut = omni_data[
            (omni_data["Epoch"] >= trange[0]) & (omni_data["Epoch"] <= trange[1])
        ]
        omni_cut.rename(
            columns={
                "BX_GSE": "B_xgsm",
            ...
            inplace=True,
        )  # Rename columns to match mms_test
    ...
    ax[0].fill_between(
        predict_cut["Epoch"],
        predict_cut["B_xgsm"] + predict_cut["B_xgsm_sig"],
        predict_cut["B_xgsm"] - predict_cut["B_xgsm_sig"],
        color=sw.c1,
        alpha=0.2,
    )
    ...
    ax[0].set_title(
        title + " (" + trange[0].strftime("%Y-%m-%d") + ")", fontsize=14
    )  # Set the title
    ...
    ax[1].fill_between(
        predict_cut["Epoch"],
        predict_cut["B_ygsm"] + predict_cut["B_ygsm_sig"],
        predict_cut["B_ygsm"] - predict_cut["B_ygsm_sig"],
        color=sw.c3,
        alpha=0.2,
    )
    ...
    ax[2].fill_between(
        predict_cut["Epoch"],
        predict_cut["B_zgsm"] + predict_cut["B_zgsm_sig"],
        predict_cut["B_zgsm"] - predict_cut["B_zgsm_sig"],
        color=sw.c4,
        alpha=0.2,
    )
    ...
    ax[3].fill_between(
        predict_cut["Epoch"],
        predict_cut["Vi_xgse"] + predict_cut["Vi_xgse_sig"],
        predict_cut["Vi_xgse"] - predict_cut["Vi_xgse_sig"],
        color=sw.c1,
        alpha=0.2,
    )
    ...
    ax[4].fill_between(
        predict_cut["Epoch"],
        predict_cut["Vi_ygse"] + predict_cut["Vi_ygse_sig"],
        predict_cut["Vi_ygse"] - predict_cut["Vi_ygse_sig"],
        color=sw.c3,
        alpha=0.2,
    )
    ...
    ax[5].fill_between(
        predict_cut["Epoch"],
        predict_cut["Vi_zgse"] + predict_cut["Vi_zgse_sig"],
        predict_cut["Vi_zgse"] - predict_cut["Vi_zgse_sig"],
        color=sw.c4,
        alpha=0.2,
    )
    ...
    ax[6].plot(
        predict_cut["Epoch"], predict_cut["Ni"], color=sw.c5, alpha=0.75, linestyle="--"
    )
    ax[6].fill_between(
        predict_cut["Epoch"],
        predict_cut["Ni"] + predict_cut["Ni_sig"],
        predict_cut["Ni"] - predict_cut["Ni_sig"],
        color=sw.c5,
        alpha=0.2,
    )
    ...
    ticks = [
        trange[0],
        trange[0] + quart_delt,
        trange[0] + 2 * quart_delt,
        trange[0] + 3 * quart_delt,
        trange[1],
    ]
    tickstamp = [
        trange[0].timestamp(),
        (trange[0] + quart_delt).timestamp(),
        (trange[0] + 2 * quart_delt).timestamp(),
        (trange[0] + 3 * quart_delt).timestamp(),
        trange[1].timestamp(),
    ]
    X_GSE_ticks = np.interp(
        tickstamp,
        (mms_data_cut["Epoch"] - pd.Timestamp("1970-01-01 00:00:00+0000"))
        // pd.Timedelta("1s"),
        mms_data_cut["R_xgse"],
    )
    Y_GSE_ticks = np.interp(
        tickstamp,
        (mms_data_cut["Epoch"] - pd.Timestamp("1970-01-01 00:00:00+0000"))
        // pd.Timedelta("1s"),
        mms_data_cut["R_ygse"],
    )
    Z_GSE_ticks = np.interp(
        tickstamp,
        (mms_data_cut["Epoch"] - pd.Timestamp("1970-01-01 00:00:00+0000"))
        // pd.Timedelta("1s"),
        mms_data_cut["R_zgse"],
    ...
    ax[6].text(
        -0.25,
        -3.775,
        trange[0].strftime("%Y-%m-%d") + "\nGSE X\nGSE Y\nGSE Z",
        transform=ax[3].transAxes,
        fontsize=14,
    )
    if save:
        plt.savefig(
            datapath + savename + "." + filetype, transparent=False, bbox_inches="tight"
        )
        plt.close()
    else:
        plt.show()
...
def wind_plot(
    trange, mfi_df, swe_df, savename, title="MMS", save=False, filetype="png"
):
    ...
    ax[1].tick_params(axis="y", which="major", labelsize=14)
    ax[1].legend(
        labels=["Y", "Z"], loc="center left", bbox_to_anchor=(1, 0.5), frameon=False
    )
    ...
    ax[2].tick_params(axis="y", which="major", labelsize=14)
    ax[2].legend(
        labels=["X"], loc="center left", bbox_to_anchor=(1, 0.5), frameon=False
    )
    ...
    X_GSE_ticks = np.interp(
        tickstamp,
        (mfi_df["Epoch"] - pd.Timestamp("1970-01-01 00:00:00+0000"))
        // pd.Timedelta("1s"),
        mfi_df["R_xgse"],
    )
    Y_GSE_ticks = np.interp(
